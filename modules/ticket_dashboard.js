...
async function updateLivePanel(client, guild, config) {
    ...
    try {
        ...
        logger.warn('Failed to update live ticket panel', { error: error.message });
        return false;
    }
}
...
export default {
    async execute(interaction, config, client) {
        try {
            ...
            collector.on('collect', async selectInteraction => {
                ...
                try {
                    ...
                    if (error instanceof TitanBotError) {
                        ...
                        logger.error('Unexpected ticket config menu error', { error: error.message });
                    }
                    const errorMessage =
            ...
            buttonCollector.on('collect', async btnInteraction => {
                try {
                    ...
                    if (error instanceof TitanBotError) {
                        ...
                        logger.error('Unexpected ticket config button error', { error: error.message });
                    }
                    const errorMessage =
                        error instanceof TitanBotError
                            ? error.userMessage || 'An error occurred while processing your selection.'
            ...
            logger.error('Unexpected error in ticket_dashboard', { error: error.message });
            throw new TitanBotError(
...
async function handlePanelMessage(selectInteraction, rootInteraction, guildConfig, guildId, client) {
        ...
        .addComponents(
            new ActionRowBuilder().addComponents(
                new TextInputBuilder()
                    ...
                    .setValue(guildConfig.ticketPanelMessage || 'Click the button below to create a support ticket.')
                    .setMaxLength(2000)
                    .setMinLength(1)
                    .setRequired(true)
    ...
    const submitted = await selectInteraction
        .awaitModalSubmit({
            filter: i => i.customId === 'ticket_cfg_panel_msg' && i.user.id === selectInteraction.user.id,
            time: 120_000,
        })
        .catch(() => null);
...
async function handleButtonLabel(selectInteraction, rootInteraction, guildConfig, guildId, client) {
    ...
    const submitted = await selectInteraction
        .awaitModalSubmit({
            filter: i => i.customId === 'ticket_cfg_btn_label' && i.user.id === selectInteraction.user.id,
            time: 120_000,
        })
        .catch(() => null);
...
async function handleOpenCategory(selectInteraction, rootInteraction, guildConfig, guildId, client) {
    ...
    catCollector.on('collect', async catInteraction => {
        ...
        await catInteraction.followUp({
            embeds: [successEmbed('✅ Open Category Updated', `New tickets will now be created in **${category.name}**.`)],
            flags: MessageFlags.Ephemeral,
        });
    ...
    catCollector.on('end', (collected, reason) => {
        if (reason === 'time' && collected.size === 0) {
            selectInteraction
                .followUp({
                    embeds: [errorEmbed('Timed Out', 'No category was selected. The setting was not changed.')],
                    flags: MessageFlags.Ephemeral,
                })
                .catch(() => {});
        }
...
async function handleClosedCategory(selectInteraction, rootInteraction, guildConfig, guildId, client) {
    ...
    catCollector.on('collect', async catInteraction => {
        ...
        await catInteraction.followUp({
            embeds: [successEmbed('✅ Closed Category Updated', `Closed tickets will now be moved to **${category.name}**.`)],
            flags: MessageFlags.Ephemeral,
        });
...
async function handleMaxTickets(selectInteraction, rootInteraction, guildConfig, guildId, client) {
    ...
    const submitted = await selectInteraction
        .awaitModalSubmit({
            filter: i => i.customId === 'ticket_cfg_max_tickets' && i.user.id === selectInteraction.user.id,
            time: 120_000,
        })
        .catch(() => null);
    ...
    if (guildConfig.ticketPanelChannelId) {
        try {
            const panelChannel = await client.guilds.cache
                .get(guildId)
                ?.channels.fetch(guildConfig.ticketPanelChannelId)
                .catch(() => null);
            if (panelChannel) {
                if (guildConfig.ticketPanelMessageId) {
                    const panelMessage = await panelChannel.messages
                        .fetch(guildConfig.ticketPanelMessageId)
                        .catch(() => null);
            ...
            logger.warn('Could not delete ticket panel message', { error: panelDeleteError.message });
        }
    }
